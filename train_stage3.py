...
def main():
    ...
    source_featuremodel = torch.load('stage2step2.pth')
    source_featuremodel.eval()

    
    model = torch.load('stage2step2.pth', map_location='cpu')
    ...
    trainloader = data.DataLoader(
        RaincouverDataSet(args.data_dir, args.data_list, max_iters=args.num_steps * args.iter_size * args.batch_size,
    ...
    for i_iter in range(0, args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            labels1 = labels1[0,:,:]
            labels1[labels1==2]=1
            labels1 = np.asarray(labels1, np.float32) #For mse loss
            labels1 = torch.from_numpy(labels1)
            labels1 = Variable(labels1).cuda(gpu)
            ...
            #BCE loss
            loss_seg2 = bce_loss(out[0,0,:,:],labels1)

            model_loss = 0

            
            for paramA, paramB in zip(model.parameters(), source_featuremodel.parameters()):
                #model_loss = model_loss + torch.sum(torch.abs(paramA - paramB.detach()))
                model_loss = model_loss + F.mse_loss(paramA, paramB.detach())
            
            loss = loss_seg2 +0.1* model_loss # Pseudo labels training
